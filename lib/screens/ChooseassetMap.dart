...
class assetOnMap extends StatefulWidget {
  const assetOnMap({Key? key}) : super(key: key);

  @override
  _assetOnMapState createState() => _assetOnMapState();
}

class _assetOnMapState extends State<assetOnMap> {
  late MapboxMapController mapController;
  List<LatLng> polygon = [];
  late Fill assetPolygonFill;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text('Draw asset on Map'),
        actions: [
          Center(
            child: Container(
                ...
                child: MapboxMap(
                    ...
                    onMapClick:
                      ...
                      if (_polygonAdded) {
                        ...
                        if (polygon.length == 3) {
                          assetPolygonFill = await mapController.addFill(
                            FillOptions(
                              fillColor: "#2596be",
                        ...
                        if (polygon.length > 3) {
                          ...
                          //print(assetPolygonFill.options.geometry.toString());
                        }
                      }
                    }),
              ),
