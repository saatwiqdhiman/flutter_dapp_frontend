...
import 'package:asset_registration/constant/loadingScreen.dart';
import 'package:asset_registration/screens/ChooseassetMap.dart';
import 'package:asset_registration/screens/viewassetDetails.dart';
import 'package:asset_registration/widget/asset_container.dart';
...
class _UserDashBoardState extends State<UserDashBoard> {
  ...
  late String fileName,
      ...
      document,
      description;
  List<List<dynamic>> assetInfo = [];
  List<List<dynamic>> receivedRequestInfo = [];
  List<List<dynamic>> sentRequestInfo = [];
  List<dynamic> prices = [];
  List<Menu> menuItems = [
    Menu(title: 'Dashboard', icon: Icons.dashboard),
    Menu(title: 'Add assets', icon: Icons.add_chart),
    Menu(title: 'My Assets', icon: Icons.landscape_rounded),
    Menu(title: 'asset Gallery', icon: Icons.landscape_rounded),
    Menu(title: 'Received Request', icon: Icons.request_page_outlined),
    Menu(title: 'Sent asset Request', icon: Icons.request_page_outlined),
    ...
    pdf.addPage(pw.Page(
        ...
        build: (pw.Context context) {
          return pw.Column(
              ...
              children: [
                pw.Center(
                  child: pw.Text('Asset Details'),
                ),
                pw.SizedBox(height: 10),
                pw.Divider(),
                pw.Center(
                  child: pw.Text('Asset Name: ' + fileName),
                ),
                pw.Divider(),
                ...
                pw.Table(columnWidths: {
                  ...
                  tableRow("1)Name: ", fileName),
                  tableRowSizedBox(),
                  tableRow("2)Asset Type: ", fileType),
                  tableRowSizedBox(),
                  tableRow("3)Category:", category),
                  tableRowSizedBox(),
                  tableRow("4)Description :", description),
                  tableRowSizedBox(),
                  tableRow("5)Content Hash :", contentHash),
                  tableRowSizedBox(),
                  tableRow("6)Asset Price:",assetPrice),
                  tableRowSizedBox(),
                  tableRow("7)Document Registration Date:", date),
                  tableRowSizedBox(),
                ]),
                pw.Divider(height: 2),
                pw.Divider(height: 2),
  ...
  getassetInfo() async {
    ...
    if (connectedWithMetamask)
      assetList = await model2.myAllassets();
    else
      assetList = await model.myAllassets();
  ...
  getMySentRequest() async {
    ...
    for (int i = 0; i < requestList.length; i++) {
      if (connectedWithMetamask) {
        ...
        pri = await model2.assetPrice(temp[3]);
      } else {
        temp = await model.requestInfo(requestList[i]);
        pri = await model.assetPrice(temp[3]);
      }
      tempPrice.add(pri);
      allInfo.add(temp);
    }
      ...
      child: Container(
        ...
        child: GridView.builder(
          ...
          gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
              ...
              crossAxisSpacing: 20,
              mainAxisSpacing: 20),
          itemCount: assetGall.length,
          itemBuilder: (context, index) {
            return assetWid2(
                assetGall[index][10],
                assetGall[index][1].toString(),
                assetGall[index][2].toString(),
                assetGall[index][3].toString(),
                assetGall[index][9] == userInfo[0],
                assetGall[index][8], () async {
              SmartDialog.showLoading();
              try {
                if (connectedWithMetamask)
                  await model2.sendRequestToBuy(assetGall[index][0]);
                else
                  await model.sendRequestToBuy(assetGall[index][0]);
              ...
              List<String> description =
                  assetGall[index][4].toString().split('|');
              print(description);
              AssetInfo assetinfo = AssetInfo(
                  assetGall[index][1].toString(),
                  assetGall[index][2].toString(),
                  assetGall[index][3].toString(),
                  assetGall[index][5].toString(),
                  assetGall[index][6].toString(),
                  assetGall[index][7].toString(),
                  assetGall[index][8],
                  assetGall[index][9].toString(),
                  assetGall[index][10]);
              Navigator.push(
                  context,
                  MaterialPageRoute(
                      builder: (context) => viewassetDetails(
                            allLatitude: description[0],
                            allLongitude: description[1],
                            assetinfo: assetinfo,
                          )));
            });
          },
      ...
      child: Container(
        ...
        child: GridView.builder(
          ...
          itemBuilder: (context, index) {
            return assetWid(
                assetInfo[index][10],
                assetInfo[index][1].toString(),
                assetInfo[index][2].toString(),
                assetInfo[index][3].toString(),
                assetInfo[index][8],
                () => confirmDialog('Are you sure to make it on sell?', context,
                        () async {
                      SmartDialog.showLoading();
                      if (connectedWithMetamask)
                        await model2.makeForSell(assetInfo[index][0]);
                      else
                        await model.makeForSell(assetInfo[index][0]);
                              ...
                              if (isFileupload) {
                                if (connectedWithMetamask)
                                  ...
                                  await model.addasset(
                                      ...
                                      contentHash,
                                      docUrl);
                                showToast("asset Successfully Added",
                                    context: context,
              ...
              itemBuilder: (BuildContext context, int index) {
                return MenuItemTile(
                  ...
                  onTap: () {
                    ...
                    if (index == 2) getassetInfo();
                    if (index == 3) getassetGallery();
